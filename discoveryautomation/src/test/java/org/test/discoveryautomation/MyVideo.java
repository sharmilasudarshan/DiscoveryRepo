...
public class MyVideo {
	...
	public static void Video() throws InterruptedException {
		...
		wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath(discovery_config.getProperty("ss_fav_vidoes"))));
		
		List<WebElement> lt1 = webDriver.findElements(By.xpath(discovery_config.getProperty("ss_fav_vidoes")));
		System.out.println("Size : " + lt1.size());

		if (lt1.size() == no_videos_fav) {
			System.out.println("Same number of videos added as Favorite from recommended section are displayed in My Video's Favorite shows");
		} else {
			flag = false;
			System.out.println("Same number of videos added as Favorite from recommended section are not displayed in My Video's Favorite shows");
		}
		...
		for (int i = 1; i <= lt1.size(); i++) {
			...
			WebElement element_title=webDriver.findElement(By.xpath("(//h2[contains(text(),'Favorite Shows')]//ancestor::section//div[@class='carousel-tile-wrapper carousel__tileWrapper'])["+i+"]//h3/div"));
			act.moveToElement(element_title).build().perform();
			WebElement element_desc=webDriver.findElement(By.xpath("(//h2[contains(text(),'Favorite Shows')]//ancestor::section//div[@class='carousel-tile-wrapper carousel__tileWrapper'])["+i+"]//div[@class='showTileSquare__description']/div"));
			act.moveToElement(element_desc).build().perform();

			wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//h2[contains(text(),'Favorite Shows')]//ancestor::section//div[@class='carousel-tile-wrapper carousel__tileWrapper']["+i+"]")));

			st_af_add.add(element_title.getText());
			sd_af_add.add(element_desc.getText());

		}
		System.out.println(st_af_add);
		System.out.println(sd_af_add);



		if (st.equals(st_af_add) && sd.equals(sd_af_add)) {
			System.out.println("Titles of the added recommended videos match with the favorite videos ");
			System.out.println("Descriptions of the added recommended videos match with the favorite videos ");
		} else {
			flag = false;
			System.out.println("Titles of the added recommended videos doesn't match with the favorite videos ");
			System.out.println("Descriptions of the added recommended videos doesn't match with the favorite videos ");
		}
	...
	public void initiaLize() throws Exception {
		...
		discovery_config.load(new FileInputStream(System.getProperty("user.dir")
				.concat("/src/test/resources/discovery_config.properties")));
